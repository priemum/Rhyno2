...
if(!user) return client.createMessage(msg.channel.id, lang_messages[0].credits_author[lang].replace('[username]', msg.author.username).replace('[coins]', credits.credits))
if(user.bot) return client.createMessage(msg.channel.id, lang_messages[0].bots_dont[lang])
if(user.id === msg.author.id) return client.createMessage(msg.channel.id, lang_messages[0].credits_author[lang].replace('[username]', msg.author.username).replace('[coins]', credits.credits))
     let creditss = await top.get(user.id)
if(creditss.blacklist === true) return client.createMessage(msg.channel.id, lang_messages[0].thisblacklist[lang])
if(!args[1]) return client.createMessage(msg.channel.id, lang_messages[0].credits_user[lang].replace('[username]', user.username).replace('[coins]', creditss.credits))
if(Number(args[1]) > Number(credits.credits)) return client.createMessage(msg.channel.id, lang_messages[0].you_dont_have[lang].replace('[coins]',credits.credits)) 

  
fetch(('https://123123h.glitch.me/api/v1/captcha') , {method: 'GET', headers: { 'Content-Type': 'application/json' }, referrerPolicy: "no-referrer"}).then(async ress =>{
